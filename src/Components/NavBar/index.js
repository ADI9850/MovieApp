...
const NavBar = () => {
  ...
  const getSuggestions = (value) => {
    setQuery(value);
    fetch(
      `https://api.themoviedb.org/3/search/movie?api_key=${key}&language=en-US&query=${value}`
    )
      .then((res) => res.json())
  ...
  return (
    <div className={`navbar ${menuOpen ? "show-menu" : ""}`}>
      <h2>MovieDb</h2>
            ...
            <div className="suggestions">
              {suggestions.map((sugg) => (
                <div
                  ...
                  key={sugg.id}
                  onClick={() => ClickMovieBtn(sugg.id)}
                >
                  <img
                    src={`https://image.tmdb.org/t/p/w500/${sugg.poster_path}`}
                    alt={sugg.title}
                  />
                  <div>{sugg.title}</div>
                </div>
